...
func sumChanger(sumChan chan int, barrier chan bool, array []int) {
	var sum int
	j := 0
	for {
		if j > 0 {
			random := rand.Intn(len(array))
			isBigger := <-barrier
...
func main() {
	...
	go sumChanger(sumChan1, barrier1, array1)
	go sumChanger(sumChan2, barrier2, array2)
	go sumChanger(sumChan3, barrier3, array3)
